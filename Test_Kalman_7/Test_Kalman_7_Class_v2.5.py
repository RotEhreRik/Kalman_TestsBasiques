...
def User2Rad(userUsesDegree: bool, angle: float) -> float:
    return np.deg2rad(angle) if userUsesDegree else angle


def Rad2User(userUsesDegree: bool, angle: float) -> float:
...
class SimulationConfig:
    ...
    def __init__(
        ...
        self.trueInitialAlphaRad = User2Rad(self.angleUnitIsDegree, trueInitialAlpha)
        self.trueInitialAlphadotRad = User2Rad(self.angleUnitIsDegree, trueInitialAlphadot)
        self.trueInitialBiasXRad = User2Rad(self.angleUnitIsDegree, trueInitialBiasX)
        self.trueInitialBiasYRad = User2Rad(self.angleUnitIsDegree, trueInitialBiasY)
        self.trueInitialBiasZRad = User2Rad(self.angleUnitIsDegree, trueInitialBiasZ)

        # if self.angleUnitIsDegree:
        #     self.trueInitialAlpha = np.deg2rad(trueInitialAlpha)
        #     self.trueInitialAlphaDot = np.deg2rad(trueInitialAlphaDot)
        #     self.trueInitialBiasX = np.deg2rad(trueInitialBiasX)
        #     self.trueInitialBiasY = np.deg2rad(trueInitialBiasY)
        #     self.trueInitialBiasZ = np.deg2rad(trueInitialBiasZ)
        # else:
        ...
        self.measurementGyroNoiseStdRad = User2Rad(self.angleUnitIsDegree, measurementGyroNoiseStd)
        # if self.angleUnitIsDegree:
    ...
    def alphaToQuaternion(self, alphaRad):
        """
        ...
        """
        alphaArrayRad = np.asarray(alphaRad, dtype=float)
        # alphaRadArray = np.deg2rad(alphaArrayRad) if self.angleUnitIsDegree else alphaArrayRad
        halfAlphaArrayRad = 0.5 * alphaArrayRad

        quaternionArray = np.zeros((alphaArrayRad.size, 4), dtype=float)
        quaternionArray[:, 0] = np.cos(halfAlphaArrayRad)  # qw
        quaternionArray[:, 1] = 0.0  # qx
        quaternionArray[:, 2] = np.sin(halfAlphaArrayRad)  # qy
        quaternionArray[:, 3] = 0.0  # qz

        if np.isscalar(alphaRad):
            return quaternionArray[0]
        return quaternionArray

    def setAngleAccelerationProfile(self, alphaAccelerationProfile: np.ndarray = [[0.0, 0.0]]):
        ...
        alphaAccelerationProfileRad = np.zeros_like(alphaAccelerationProfile)
        alphaAccelerationProfileRad[:, 0] = alphaAccelerationProfile[:, 0]
        alphaAccelerationProfileRad[:, 1] = User2Rad(self.angleUnitIsDegree, alphaAccelerationProfile[:, 1])
        self.alphaAccelerationProfileRad = alphaAccelerationProfileRad
        # if self.angleUnitIsDegree:
        #     self.alphaAccelerationProfile = np.deg2rad(alphaAccelerationProfile)
        # else:
    ...
    def generateTrueValuesAndMeasurements(self):
        ...
        Retourne : (timeArray, trueAlphaArrayRad, trueAlphadotArrayRad,
        ...
        trueAlphaArrayRad = np.zeros(self.sampleSize, dtype=float)
        trueAlphadotArrayRad = np.zeros(self.sampleSize, dtype=float)

        currentAlphaRad = self.trueInitialAlphaRad
        currentAlphadotRad = self.trueInitialAlphadotRad

        alphadotdotsRad = self.alphaAccelerationProfileRad
        ...
        for indexTime in range(self.sampleSize):
            ...
            if ratio > alphadotdotsRad[percentIndex + 1][0]:
                percentIndex = percentIndex + 1
            currentAlphadotdotRad = alphadotdotsRad[percentIndex][1]

            # if currentTime < 20.0:
            #     currentAlphadotdotRad = 0.0
            # elif currentTime < 40.0:
            #     currentAlphadotdotRad = 0.5
            # elif currentTime < 60.0:
            #     currentAlphadotdotRad = -0.3
            # else:
            #     currentAlphadotdotRad = 0.0

            # currentAlphadotdotRad = 0.0

            currentAlphadotRad = currentAlphadotRad + currentAlphadotdotRad * self.timeStep
            currentAlphaRad = currentAlphaRad + currentAlphadotRad * self.timeStep
            trueAlphaArrayRad[indexTime] = currentAlphaRad
            trueAlphadotArrayRad[indexTime] = currentAlphadotRad

        trueQuaternionArray = self.alphaToQuaternion(trueAlphaArrayRad)

        # trueAlphaRadArray = (
        #     np.deg2rad(trueAlphaArrayRad) if self.angleUnitIsDegree else trueAlphaArrayRad
        # )
        ...
        measuredAccelXArray = -self.gravity * np.sin(trueAlphaArrayRad) + accelNoiseX
        measuredAccelYArray = np.zeros(self.sampleSize, dtype=float) + accelNoiseY
        measuredAccelZArray = self.gravity * np.cos(trueAlphaArrayRad) + accelNoiseZ

        gyroNoiseXRad = np.random.normal(0.0, self.measurementGyroNoiseStdRad, self.sampleSize)
        gyroNoiseYRad = np.random.normal(0.0, self.measurementGyroNoiseStdRad, self.sampleSize)
        gyroNoiseZRad = np.random.normal(0.0, self.measurementGyroNoiseStdRad, self.sampleSize)

        measuredGyroXArrayRad = 0.0 + self.trueInitialBiasXRad + gyroNoiseXRad
        measuredGyroYArrayRad = trueAlphadotArrayRad + self.trueInitialBiasYRad + gyroNoiseYRad
        measuredGyroZArrayRad = 0.0 + self.trueInitialBiasZRad + gyroNoiseZRad

        trueAlphaArray = Rad2User(self.angleUnitIsDegree, trueAlphaArrayRad)
        trueAlphadotArray = Rad2User(self.angleUnitIsDegree, trueAlphadotArrayRad)
        measuredGyroXArray = Rad2User(self.angleUnitIsDegree, measuredGyroXArrayRad)
        measuredGyroYArray = Rad2User(self.angleUnitIsDegree, measuredGyroYArrayRad)
        measuredGyroZArray = Rad2User(self.angleUnitIsDegree, measuredGyroZArrayRad)
        # if self.angleUnitIsDegree:
        #     trueAlphaArray = np.rad2deg(trueAlphaArrayRad)
        #     trueAlphaDotArray = np.rad2deg(trueAlphadotArrayRad)
        #     measuredGyroXArray = np.rad2deg(measuredGyroXArrayRad)
        #     measuredGyroYArray = np.rad2deg(measuredGyroYArrayRad)
        #     measuredGyroZArray = np.rad2deg(measuredGyroZArrayRad)
        # else:
        #     trueAlphaArray = trueAlphaArrayRad
        #     trueAlphaDotArray = trueAlphadotArrayRad
        #     measuredGyroXArray = measuredGyroXArrayRad
        #     measuredGyroYArray = measuredGyroYArrayRad
        #     measuredGyroZArray = measuredGyroZArrayRad
...
class UkfModel:
    ...
    def __init__(
        ...
        self.lastGyroInputRad = np.zeros(3, dtype=float)

    def setCurrentGyroMeasurement(self, gyroMeas):
        gyroMeasRad = User2Rad(simConfig.angleUnitIsDegree, gyroMeas)
        self.lastGyroInputRad = np.array(gyroMeasRad, dtype=float)
    ...
    def quaternionToEulerRad(self, q):
        q = self.normalizeQuaternion(q)
        w, x, y, z = q
        ...
        RollPitchYawRad = np.array([roll, pitch, yaw], dtype=float)
        return RollPitchYawRad
        # return Rad2User(self.angleUnitIsDegree, RollPitchYawRad)
    ...
    def stateTransitionFunction(self, x, dt):
        ...
        omegaRad = self.lastGyroInputRad - bRad
        wx, wy, wz = omegaRad
    ...
    def measurementFunction(self, x):
        ...
        omegaBodyRad = self.lastGyroInputRad - bRad
        gyroPredRad = omegaBodyRad + bRad

        return np.hstack((accelPred, gyroPredRad))
...
class UkfParams:
    ...
    def __init__(
        ...
        self.supposedInitialBiasXRad = User2Rad(self.angleUnitIsDegree, supposedInitialBiasX)
        self.supposedInitialBiasYRad = User2Rad(self.angleUnitIsDegree, supposedInitialBiasY)
        self.supposedInitialBiasZRad = User2Rad(self.angleUnitIsDegree, supposedInitialBiasZ)
        # self.memorisedInitialBiasX = supposedInitialBiasX
        # self.memorisedInitialBiasY = supposedInitialBiasY
        # self.memorisedInitialBiasZ = supposedInitialBiasZ
        # if self.angleUnitIsDegree:
        #     self.supposedInitialBiasX = np.deg2rad(supposedInitialBiasX)
        #     self.supposedInitialBiasY = np.deg2rad(supposedInitialBiasY)
        #     self.supposedInitialBiasZ = np.deg2rad(supposedInitialBiasZ)
        # else:
        ...
        self.measurementGyroNoiseStdRad = simConfig.measurementGyroNoiseStdRad

        self.processQuaternionNoiseStd = processQuaternionNoiseStd
        # self.processAlphadotNoiseStd = processAlphadotNoiseStd

        self.processBiasNoiseStdRad = User2Rad(self.angleUnitIsDegree, processBiasNoiseStd)
    ...
    def getConstructorAttrs(cls, base: "UkfParams"):
        constructorAttrs = {
            ...
            "supposedInitialBiasX": Rad2User(base.angleUnitIsDegree, base.supposedInitialBiasXRad),
            "supposedInitialBiasY": Rad2User(base.angleUnitIsDegree, base.supposedInitialBiasYRad),
            "supposedInitialBiasZ": Rad2User(base.angleUnitIsDegree, base.supposedInitialBiasZRad),
            "processQuaternionNoiseStd": base.processQuaternionNoiseStd,
            # "processAlphadotNoiseStd": base.processAlphadotNoiseStd,
            "processBiasNoiseStd": Rad2User(base.angleUnitIsDegree, base.processBiasNoiseStdRad),
    ...
    def fromBase(cls, base: "UkfParams", **overrides) -> "UkfParams":
        ...
        #     "supposedInitialQuaternion": base.supposedInitialQuaternion,
        #     # "supposedInitialAlphadot": base.supposedInitialAlphadot,
        #     "supposedInitialBiasX": Rad2User(base.angleUnitIsDegree, base.supposedInitialBiasX),
        #     "supposedInitialBiasY": Rad2User(base.angleUnitIsDegree, base.supposedInitialBiasY),
        #     "supposedInitialBiasZ": Rad2User(base.angleUnitIsDegree, base.supposedInitialBiasZ),
        #     "processQuaternionNoiseStd": base.processQuaternionNoiseStd,
        #     # "processAlphadotNoiseStd": base.processAlphadotNoiseStd,
        #     "processBiasNoiseStd": Rad2User(base.angleUnitIsDegree, base.processBiasNoiseStd),
        #     "processInitialConfidenceStd": base.processInitialConfidenceStd,
        #     "label": base.label,
        # }
...
class UkfRunner:
    ...
    def run(
        ...
        ukf.x = np.hstack((
            initialQuaternion,
            np.array([
                params.supposedInitialBiasXRad,
                params.supposedInitialBiasYRad,
                params.supposedInitialBiasZRad,
            ], dtype=float)
        ))
        ...
        ukf.R = np.diag([
            ...
            params.measurementAccelNoiseStd ** 2,
            params.measurementAccelNoiseStd ** 2,
            params.measurementGyroNoiseStdRad ** 2,
            params.measurementGyroNoiseStdRad ** 2,
            params.measurementGyroNoiseStdRad ** 2,
        ])
        ...
        ukf.Q = np.diag([
            ...
            params.processQuaternionNoiseStd ** 2,
            params.processQuaternionNoiseStd ** 2,
            params.processBiasNoiseStdRad ** 2,
            params.processBiasNoiseStdRad ** 2,
            params.processBiasNoiseStdRad ** 2,
        ])
        ...
        for accelX, accelY, accelZ, gyroX, gyroY, gyroZ in zip(
            ...
            bEst = Rad2User(params.angleUnitIsDegree, bEstRad)
            eulerEstRad = model.quaternionToEulerRad(qEst)
            eulerEst = Rad2User(params.angleUnitIsDegree, eulerEstRad)
...
if __name__ == "__main__":
    ...
    simConfig = SimulationConfig(
        ...
        measurementAccelNoiseStd=0.1,
        measurementGyroNoiseStd=np.deg2rad(5.0),
    )

    # trueBiases = np.full(simConfig.sampleSize,[[simConfig.trueInitialBiasX, simConfig.trueInitialBiasY,simConfig.trueInitialBiasZ]])
    trueBiases = np.tile(
        [
            simConfig.trueInitialBiasXRad,
            simConfig.trueInitialBiasYRad,
            simConfig.trueInitialBiasZRad,
        ],
    ...
    simConfig.setAngleAccelerationProfile(
        [
            [0.0, np.deg2rad(0.0)],
            [.1, np.deg2rad(angleAccel)],
            [.3, np.deg2rad(-angleAccel)],
            [.5, np.deg2rad(-angleAccel)],
            [.7, np.deg2rad(angleAccel)],
            [.9, np.deg2rad(0.0)],
        ]
    )
    ...
    axes[5].plot(timeArray, np.rad2deg([0] * simConfig.sampleSize), 'k--', label="trueAlphaArray")
    axes[6].plot(timeArray, np.rad2deg(trueAlphaArray), 'k--', label="trueAlphaArray")
    axes[7].plot(timeArray, np.rad2deg([0] * simConfig.sampleSize), 'k--', label="trueAlphaArray")
