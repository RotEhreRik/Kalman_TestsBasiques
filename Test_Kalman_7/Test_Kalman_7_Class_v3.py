...
class UkfRunner:
    ...
    def run(
        ...
        for accelX, accelY, accelZ, gyroX, gyroY, gyroZ in zip(
            ...
            ukf.update(np.array([accelX, accelY, accelZ], dtype=float))
            ukf.x[0:4] = model.normalizeQuaternion(ukf.x[0:4])
...
if __name__ == "__main__":
    ...
    paramsRef = UkfParams(simConfig, processBiasNoiseStd=0.1)
    ...
    # --- Cellule 4 : affichage ---
    fig, axes = plt.subplots(10, 1, figsize=(12, 24), sharex=True)
    for res in results:
        ...
        axes[4].plot(timeArray, res.estimatedEulerDegArray[:,0], label="estimatedEulerArray "+res.label)
        axes[5].plot(timeArray, res.estimatedEulerDegArray[:,1], label="estimatedEulerArray "+res.label)
        axes[6].plot(timeArray, res.estimatedEulerDegArray[:,2], label="estimatedEulerArray "+res.label)
        axes[7].plot(timeArray, res.estimatedBiasArray[:,0], label="estimatedBiasArray "+res.label)
        axes[8].plot(timeArray, res.estimatedBiasArray[:,1], label="estimatedBiasArray "+res.label)
        axes[9].plot(timeArray, res.estimatedBiasArray[:,2], label="estimatedBiasArray "+res.label)
    ...
    axes[4].plot(timeArray, [0]*simConfig.sampleSize, 'k--', label="trueAlphaArray")
    axes[5].plot(timeArray, trueAlphaArray, 'k--', label="trueAlphaArray")
    axes[6].plot(timeArray, [0]*simConfig.sampleSize, 'k--', label="trueAlphaArray")
    axes[7].plot(timeArray, trueBiases[:,0], 'k--', label="trueBiases")
    axes[8].plot(timeArray, trueBiases[:,1], 'k--', label="trueBiases")
    axes[9].plot(timeArray, trueBiases[:,2], 'k--', label="trueBiases")

    # titles = [np.reshape(["Quaternions (-)"]*4, (4,1)), np.reshape(["EulerDeg (°)"]*3, (3,1)), np.reshape(["Biais (°/s)"]*3, (3,1))]
    # titles = np.reshape(titles, (10, 1, 1))
    titles = ["Quaternions (-)"]*4 + ["EulerDeg (°)"]*3 + ["Biais (°/s)"]*3
    for ax, title in zip(axes, titles):
        ax.set_ylabel(title)
        ax.legend()
        ax.grid(True)

    axes[9].set_xlabel("Temps (s)")
    plt.tight_layout()
    plt.show()
